...
(function () {
  ...
  Rivington.Harvest.Game_Interpreter_pluginCommand =
  ...
  Rivington.Harvest.parseComment = function(command,args,eventId) {
    ...
    var evComments = _.filter(evCmds,(o) => {return o.code === 108 || o.code === 408;}) //find all comments within this page
    //Amongst any and all comments find the start and end of the <hvCommand> comment
    var startKey, endKey = 0;
    _.forEach(evComments,(value,key) => {
      if (value.parameters[0] == `<hv${command}>`) {startKey = key}
      if (value.parameters[0] == `</hv${command}>`) {endKey = key}
    })
    evComments = _.filter(evComments,(value,key) => {return key > startKey && key < endKey;}); //filter the comments to only everything inbetween <createHarvest>
    //build a clean harvest js object based on that comment's strings
    var hvObject = {};
    _.forEach(evComments,(o) => {
      ...
      if ($gameActors.actor(thisActor).isLearnedSkill(hvObject.skill)) {
        if ($gameParty.hasItem($dataItems[hvObject.tool],true) || !hvObject.tool) {
            ...
            if ($gameActors.actor(thisActor).jpLevel(hvObject.job) > currentJobLvl) {
              let msg = `${$dataClasses[hvObject.job].name} is now level ${$gameActors.actor(thisActor).jpLevel(hvObject.job)}!`;
              Rivington.Harvest.hvMessage(2,msg,'Saint5');
            }


        } else {
          let msg = `No one in the party has a ${$dataItems[hvObject.tool].name}.`;
          Rivington.Harvest.hvMessage(2,msg);
        }
      } else {
        let msg = `No one has learned the ${$dataSkills[hvObject.skill].name} skill.`;
        Rivington.Harvest.hvMessage(2,msg);
      }
    } else {
      let msg = `Requires ${$dataClasses[hvObject.job].name} level ${hvObject.level}.`;
      Rivington.Harvest.hvMessage(2,msg);
    }
